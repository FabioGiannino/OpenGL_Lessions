...
QuadPerlinNoise::QuadPerlinNoise()
{
    Program = new OpenGL_Program("resources/shaders/PerlinNoise.vert","resources/shaders/PerlinNoise.frag");

    std::vector<float> Vertices = {
        //Triangolo di sinistra         //Color RGB
        -0.5f,  -0.5f, 0.0f,            0.f, 0.f, 0.f,
        0.5,    -0.5f,  0.0f,           0.f, 0.f, 0.f,
        -0.5f,   0.5f,   0.0f,          0.f, 0.f, 0.f,
...
void QuadPerlinNoise::Update(float InDeltaTime)
{
    glClear(GL_COLOR_BUFFER_BIT);   //Ad ogni update puliamo il buffer 
    
    static float ElapsedTime = 0;
    ElapsedTime += InDeltaTime;
    GLfloat BaseColorLocation =  glGetUniformLocation(Program->GetProgramID(), "base_color"); 
    glUniform1f(BaseColorLocation, ElapsedTime);      
